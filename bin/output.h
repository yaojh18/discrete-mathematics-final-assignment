...
void output_as_csv(vector<Paper>& paper, vector<Author>& author,vector<vector<Connection>>& paper_connection,vector<vector<Connection>>& author_connection){
    ...
    fout<<"DOI,closeness_centrality,betweenness_centrality,group,label"<<endl;
    ...
    fout<<"source,target,distance"<<endl;
    for (int i=0;i<paper.size();i++)
        for (int j=0;j<paper[i].neighbors.size();j++)
            fout<<i<<','<<paper[i].neighbors[j]<<','<<paper[i].distance[j]<<endl;
    fout.close();
    fout.open("author.csv");
    fout<<"name,closeness_centrality,betweenness_centrality,group,label"<<endl;
    ...
    for (int i=0;i<author.size();i++)
        for (int j=0;j<author[i].neighbors.size();j++)
            fout<<i<<','<<author[i].neighbors[j]<<','<<author[i].distance[j]<<endl;
    fout.close();
